...
for whichWinDowSize=1:length(winsize)
    ...
    for i = 1:length(symbols)
        ...
        prices(date_idx, i) = stock_data.close;  % close price
    end
    ...
    % rolling window's performance
    performance = zeros(size(returns,1), 1);
    for t = window_size:length(returns)
        % use the returns from each window_size to get avg performance

        window_returns = returns(t-window_size+1:t, :);
        performance(t) = mean(window_returns(:)); % 
    end
    %
    % volatility,stochasticity
    [volatility, stochasticity] = calculate_uncertainty(returns, window_size);
    ratio = volatility ./ stochasticity;
    ...
    performance(filterstc)=[];
    
    
    % clean data
    valid_idx = ~isinf(ratio) & ~isnan(ratio) & ratio > 0;
    ratio_clean = ratio(valid_idx);
    perf_clean = performance(valid_idx);
    
    
    %remove outlier (1%,99% quantile)
        valid_idx = ~isnan(ratio) & ~isnan(performance);
    ratio_clean = ratio(valid_idx);
    perf_clean = performance(valid_idx);
    [ratio_filtered, exp_filtered] = remove_outliers(ratio_clean,perf_clean);
    ...
    subplot(1,3,1)
    mainfunc_plot_Perf_vsratio_bins_fitted_curve(x{1}, y{1},lmresults,n_bins,lmresults.Rsquared.Adjusted,color);
    title('S&P500 5yrs');xlabel('log(volstc ratio)');ylabel('performance');
...
saveas(gcf,'SP_500_winsize_100_150_erfp_vsratio.pdf');
...
% no extremevalue
stochasticity = winsorize(stochasticity, 0.01); % 1% extreme value winsorization
end
...
acf = acf(2:end); % remove lag 0

% 3. get runs test stats
...
for t = window_size:n_days
    % use the returns of each window_size
    window_returns = returns(t-window_size+1:t, :);
